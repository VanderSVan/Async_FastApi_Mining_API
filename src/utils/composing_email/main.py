...
def compose_confirm_email(email: EmailStr,
    ...
    template_body = {
        'heading': f'You have registered on the service "{settings.API_NAME}".',
        'text': 'You need to confirm your email address: ',
        'email': email,
        'expire': expire,
        'url': url
    }
...
def compose_reset_password_email(email: EmailStr,
    ...
    expire = create_expire()
    template_body = {
        'heading': f'You requested password reset on "{settings.API_NAME}".',
